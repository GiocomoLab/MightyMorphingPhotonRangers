...
if __name__ == '__main__':
    ...
    for mouse in mice:
        ...
        dirbase = "G:\\My Drive\\Figures\\TwoTower\\SFN2018\\Decoding\\%s\\" % mouse
        try:
            os.makedirs(dirbase)
        except:
        ...
        for i in range(df_sess.shape[0]):
            try:
                ...
                fname = "%s\\%s_%s_%d\\" % (dirbase,mouse,df_sess['DateFolder'].iloc[i],df_sess['SessionNumber'].iloc[i])
                print(fname)
                if os.path.isdir(fname):
                    print("skipping")
                    pass
                else:
                    decode_dict = bd.single_session(df_sess.iloc[i],save=True,dirbase=fname)
