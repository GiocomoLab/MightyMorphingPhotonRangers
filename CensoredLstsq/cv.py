...
# MP 12/14/2018
def masked_pca(data, rank, M=None, nonneg=False,maxiter=100):
    """Fit PCA or NMF while holding out a fraction of the dataset.
    """
    ...
    # create masking matrix if not passed
    if M is None:
        M = ~np.isnan(data)
    # zero masked elements
    rowMean = np.matlib.repmat(np.nanmean(data,axis=0)[np.newaxis,:],data.shape[0],1)
    ...
    #resid[:,:] = np.dot(U, Vt) - data
    #resid[:,:]=resid**2
    #train_err = np.nanmean(residS)
    #test_err = np.mean(resid[~M]**2)
    return U, Vt, train_err
